...
class Nmysql {
    ...
    */
    protected function _dataFormat($tbName,$data) {
        ...
        $ret=array();
        foreach ($data as $key=>$val) {//echo 'mgg'.'<br>';var_dump($val);
            if (!is_scalar($val)) continue; //值不是标量则跳过
            if (array_key_exists($key,$table_column)) {
                $key = $this->_addChar($key);//echo 'cgg'.'<br>';var_dump($val);
                if (is_int($val)) { 
                    $val = intval($val); 
                } elseif (is_float($val)) { 
                    $val = floatval($val); 
    ...
    */
    protected function _doQuery($sql='') {
        $this->_sql = $sql;
        $PDOstmt = self::$_dbh->prepare($this->_sql); //prepare或者query 返回一个\PDOStatement
        $PDOstmt->execute();
    ...
    * @return mixed 
    */
    public function doSql($sql='',$type=0) {
...
/*            if($re&&$type){
    ...
    */
     ...
     * @return int 受影响的行数
     */
    public function insert($tbName,array $data,$lastID=0){
